...
# ... (все остальные импорты остаются без изменений)
...
# ... (все функции create_..._document остаются без изменений)
...
def research_and_learn(topic: str) -> str:
    """Ищет информацию в интернете, создает структурированное саммари и добавляет его в базу знаний."""
    ...
    # Вызываем LLM для создания саммари
    summary = llm.invoke(summarizer_prompt).content
    logger.info("Создано саммари найденной информации.")

    # Разбиваем качественное саммари на чанки и добавляем в базу Архивариуса
...
tools = [
    Tool(
        ...
        description="Используй, чтобы исследовать новую тему, найти по ней информацию, проанализировать и сохранить ее в долгосрочную память. Принимает на вход тему для исследования."
    ),
    Tool(
        name="Archivist",
...
# (Этот блок остается без изменений, с последними строгими правилами)
...
# ... (весь код для start, handle_text_message, handle_photo_message остается без изменений) ...
